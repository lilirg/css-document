...
CSS 选择器提供了很多种方法，所以在选择要样式化的元素时，我们可以做到很精细的地步。
...
| `:indeterminate`     | 匹配未定态值的 UI 元素，通常为复选框（checkbox）。                                                                                                                                                            |
...
通用兄弟选择器（General Sibling Selector）是一种关系选择器，它选择元素的任何兄弟元素。通用兄弟选择器的语法是两个选择器之间用波浪号`~`分隔。
