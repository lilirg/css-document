...
`inset-block-start` 属性是 CSS 逻辑属性，用于设置块起始方向的定位值（在垂直书写模式下是顶部定位，在水平书写模式下是左侧定位）。
...
1. **书写模式**：在水平书写模式（`writing-mode: horizontal-tb`）下，对应 `top`
2. **垂直书写**：在垂直书写模式下，对应 `left` 或 `right`（根据书写方向）
