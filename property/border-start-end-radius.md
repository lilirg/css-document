...
`border-start-end-radius` 属性是 CSS 逻辑属性，用于设置块起始 - 行内结束方向的边框圆角（在水平书写模式下是右上角圆角）。
...
```css
.double-radius {
  border-start-end-radius: 20px 10px;
  border-start-end-style: solid;
}
```
...
1. **书写模式**：在水平书写模式（`writing-mode: horizontal-tb`）下，对应 `border-top-right-radius`
