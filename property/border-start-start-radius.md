...
`border-start-start-radius` 属性是 CSS 逻辑属性，用于设置块起始 - 行内起始方向的边框圆角（在水平书写模式下是左上角圆角）。
...
.single-radius {
  ...
  border-start-start-style: solid;
}
```
...
```css
.double-radius {
  border-start-start-radius: 20px 10px;
  border-start-start-style: solid;
}
```
...
1. **书写模式**：在水平书写模式（`writing-mode: horizontal-tb`）下，对应 `border-top-left-radius`
