...
`border-end-start-radius` 属性是 CSS 逻辑属性，用于设置块结束 - 行内起始方向的边框圆角（在水平书写模式下是左下角圆角）。
...
1. **书写模式**：在水平书写模式（`writing-mode: horizontal-tb`）下，对应 `border-bottom-left-radius`
